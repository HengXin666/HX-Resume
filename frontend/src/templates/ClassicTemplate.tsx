...
export default function ClassicTemplate({ resume, onSectionClick }: Props) {
  ...
  const { heading: headingStyle, underline: underlineStyle } = buildHeadingStyle(style.heading_style, style.primary_color);

  const renderHeading = (text: string) => (
    <div>
      <h2 style={headingStyle}>{text}</h2>
      {underlineStyle && <div style={underlineStyle} />}
    </div>
  );
  ...
  const handleClick = (key: string) => (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    // Check if the click was on a specific item (data-item-index)
  ...
  const sectionRenderers: Record<SectionKey, () => React.ReactNode> = {
    summary: () =>
      basics.summary ? (
        <div
          key="summary"
          className="resume-section resume-section--clickable"
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2px', marginBottom: '4px', fontSize: '12px', color: 'var(--resume-sub)' }}>
                  ...
                  {p.url && (
                    <a
                      ...
                      style={{ fontSize: '11px', color: 'var(--resume-link)', display: 'inline-flex', alignItems: 'center', gap: '3px', flexShrink: 0, maxWidth: '220px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
        ...
        >
          ...
          {education.map((e, i) => (
            <div key={i} className="resume-item" data-section="education" data-item-index={i}>
              <div className="resume-item-header">
                <div>
                  <span className="resume-item-title">{e.institution}</span>
                  {e.area && (
                    <span style={{ color: 'var(--resume-muted)', marginLeft: '8px' }}>
                      {e.area}
        ...
        >
          ...
          {awards.map((a, i) => (
            <div key={i} className="resume-item" data-section="awards" data-item-index={i}>
              <div className="resume-item-header">
                <div>
                  <span className="resume-item-title">{a.title}</span>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {interests.flatMap((item) => item.keywords.length > 0 ? item.keywords : [item.name]).map((k, i) => (
              <span
                ...
                style={{ background: 'var(--resume-tag-bg)', color: 'var(--resume-muted)' }}
              >
                {k}
              </span>
            ))}
  ...
  return (
    <div className="resume-page" style={pageStyle}>
      {/* Header — compact: name + contact on fewer lines */}
      <div
        style={{ marginBottom: `${style.section_gap}px`, textAlign: 'center', ...sectionClickStyle }}
        className="resume-section--clickable"
        onClick={handleClick('basics')}
      >
        <h1 style={{ color: style.primary_color, marginBottom: '2px', fontSize: '24px' }}>{basics.name || '您的姓名'}</h1>
        {basics.label && (
          <div style={{ fontSize: '14px', color: 'var(--resume-muted)', marginBottom: '6px' }}>
            {basics.label}
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '4px 12px', fontSize: '12px', color: 'var(--resume-contact)' }}>
          {basics.phone && <span>{basics.phone}</span>}
          {basics.email && <span>{basics.email}</span>}
          {basics.url && (
            <a href={basics.url} onClick={(e) => e.stopPropagation()} style={{ color: 'var(--resume-link)', textDecoration: 'none' }}>{basics.url.replace(/^https?:\/\//, '')}</a>
          )}
