...
export default function MinimalTemplate({ resume, onSectionClick }: Props) {
            ...
            <div key={i} data-section="projects" data-item-index={i} style={{ marginBottom: '14px' }}>
              ...
              {(p.keywords.length > 0 || p.url) && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2px', marginBottom: '4px', fontSize: '11px', color: 'var(--resume-sub)' }}>
                  ...
                  {p.url && (
                    <a href={p.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '11px', color: 'var(--resume-link)', display: 'inline-flex', alignItems: 'center', gap: '3px', flexShrink: 0, maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
  ...
  return (
    ...
    >
      {/* Header — ultra clean, compact */}
      <div
        style={{ textAlign: 'center', marginBottom: '16px', ...sectionClickStyle }}
        className="resume-section--clickable"
        onClick={handleClick('basics')}
      >
        <h1 style={{ fontSize: '24px', fontWeight: 300, letterSpacing: '4px', textTransform: 'uppercase', color: 'var(--resume-h2)' }}>
          {basics.name || '您的姓名'}
        </h1>
        {basics.label && (
          <div style={{ fontSize: '13px', color: 'var(--resume-dim)', marginTop: '2px', letterSpacing: '1px' }}>
            {basics.label}
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '4px 16px', marginTop: '6px', fontSize: '12px', color: 'var(--resume-dim)' }}>
          {basics.email && <span>{basics.email}</span>}
          {basics.phone && <span>{basics.phone}</span>}
          {basics.url && <span>{basics.url}</span>}
          {basics.location?.city && <span>{basics.location.city}</span>}
        </div>
        <div style={{ width: '40px', height: '1px', background: 'var(--resume-divider)', margin: '10px auto 0' }} />
      </div>
